...
final class KeyValueStore[+T](map: Ref[Map[Key, T]]) extends StateMachine[T] {
  def write = { case WriteKey(key: Key, value: T @unchecked) => map.update(_ + (key -> value)) }
  def read  = { case ReadKey(key) => map.get.map(_.get(key)) }
}

object KeyValueStore {
  def apply[R]: UIO[KeyValueStore[R]] = Ref.make(Map.empty[Key, R]).map(new KeyValueStore(_))
}
